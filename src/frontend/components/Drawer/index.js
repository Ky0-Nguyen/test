...
export default class Drawer extends Component {
  ...
  componentWillMount () {
    ...
    this.MAX_DX = drawerWidth > width(100) ? width(100) : drawerWidth
  ...
  _getCurrentDrawerWidth () {
    return this.isLeftActive ? this.styles.leftDrawer.style.left + this.MAX_DX
      : this.styles.rightDrawer.style.left - width(100)
  }
  _touchPositionCheck (gestureState) {
    ...
    if (!leftDisabled && this.isLeft && x0 <= width(100) * 0.2 && !isOpen && dx > 0) {
  ...
  _updateNativeStyles (dx) {
    ...
    this.styles.leftDrawer.style.right = width(100) - dx
    this.styles.rightDrawer.style.left = width(100) + dx
